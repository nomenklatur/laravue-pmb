  ...
  <div class="content-wrapper">
    ...
    <section class="content">
                ...
                <table class="table table-bordered">
                  ...
                  <tbody v-if="data.length">
                    <tr v-for="(item, i) in data" :key="i" >
                      ...
                      <td>
                            <a :href="'/upload_file/'+item.file.file_rekom" v-if="item.file.file_rekom!=''" target="_blank">File Rekom</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                      <td>
                            <a :href="'/upload_file/'+item.file.file_cv" v-if="item.file.file_cv!=''" target="_blank">File CV</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                      <td>
                            <a :href="'/upload_file/'+item.file.file_transkrip" v-if="item.file.file_transkrip!=''" target="_blank">File Transkrip</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                      <td>
                            <a :href="'/upload_file/'+item.file.file_ijazah" v-if="item.file.file_ijazah!=''" target="_blank">File Ijazah</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                      <td>
                            <a :href="'/upload_file/'+item.file.file_karyailmiah" v-if="item.file.file_karyailmiah!=''" target="_blank">File Karya Ilmiah</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                      <td>
                            <a :href="'/upload_file/'+item.file.file_foto" v-if="item.file.file_foto!=''" target="_blank">Foto</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                       <td>
                            <a :href="'/upload_file/'+item.file.file_ktp" v-if="item.file!=null" target="_blank">KTP</a>
                            <p v-else>Belum diunggah</p>
                      </td>
                      <td align="center">
                        <Button
                          type="primary"
                          @click="showModalFunc(item, i)"
                          ghost
                        >
                          Aktifkan
                        </Button>
     ...
     <Modal v-model="showModal" title="Pilih Status" :mask-closable="false">
      ...
      <div class="form-group">
        <label class="col-sm-4 col-form-label">Nomor Ujian</label>
        <div class="col-sm-8 d-flex align-items-center">
          <Input v-model="form.nomor" type="text" placeholder="Nomor ujian..." style="width: 8em" />
          <p style="margin-left:10px">0{{this.form.prodi}}.pmb.2021</p>
        </div>
      </div>
  ...
  methods: {
    async getParticipant(page = 1) {
      const res = await this.apiRequest(
        "get",
        `/api/pendingparticipantspasca?page=${page}&total=${this.total}`
      );

      const dataArr = res.data.data.filter(item=>{
          return item.activated == '0' && item.file != null && item.file.status == 'pending'
      })
